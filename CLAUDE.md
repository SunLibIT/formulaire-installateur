...
| Devis abonnement (`devis`) | dossier | ✅ | ✅ | PDF | **IA** (§ docs génériques) — signature, total HT=0, montant/durée vs formulaire, matériel. *(pro `dev-pro` : à ajuster)* |
| Avis d'imposition (`impots`) ³ | dossier | ✅ | — | PDF | **IA** (§ docs génériques) — pages complètes + nom cohérent + solvabilité |
| Facture d'énergie (`elec`) | dossier | ✅ | — | PDF/JPG/PNG | **IA** (§ docs génériques) — conso annuelle + PDL vs formulaire ; échéancier si « nouvelle acquisition » |
| Titre de propriété (`prop`) | dossier | ✅ | — | PDF | **IA** (§ Feature docs génériques) |
...
- **Front** : registre `DOC_VERIFY` (clé document → `docType`) — actuellement `{ prop:'TITRE_PROPRIETE', maps/'map-pro':'GOOGLE_MAPS', calep/'cal-pro':'ETUDE_INSTALLATEUR', devis:'DEVIS', impots:'AVIS_IMPOT', bilans:'BILANS', elec:'FACTURE_ELEC' }`. À l'ajout/retrait → `scheduleDocCheck(key)` → `doDocCheck(key)` POST `/api/drafts {action:'verifyid', docType, ids, refIds, form}` où `form` = `docFormContext()` (`{adresse, abonnes, duree, mensualite, batteriePhysique, pdl, nouvelleAcquisition}` — `batteriePhysique` = `iType ∈ {pvb,bat}` pour le seuil de solvabilité ; `pdl` = `#sl-pdl` ; `nouvelleAcquisition` = case `#sl-elec-newacq` de la carte Facture). Verdict dans `docVerdicts[key]` ; `renderDoc` **colore chaque vignette de fichier + affiche le motif dessus** (`_docChip(key,item,idx,vinfo)` + `docFileVinfo(key)` → tone `ok`/`warn`/`bad`), comme la CNI (`cniFileVinfo`). Coloration par fichier valable pour **tous** les documents. Blocage : `docsBlockReason()` (dans `slCreate`) si `statut==='ko'`.
...
- **Types couverts & contrôles** (tous bloquants sauf mention ; `a_verifier` / IA indisponible → non bloquant) :
  ...
  - `DEVIS` (clé `devis`, **particulier**) : `signature` · `total_ht_zero` **[BLOQUANT-DUR]** (total HT doit valoir 0, modèle abonnement) · `duree_abonnement` [CMP-FORM] · `materiel` (panneaux + onduleurs) · `lisible` = bloquants ; `montant_abonnement` [CMP-FORM] = **avertissement** (la mensualité du formulaire est une estimation). `docFormContext` fournit `duree` + `mensualite`. *(Devis pro `dev-pro` non branché — à ajuster.)*
  - `AVIS_IMPOT` (clé `impots`, **particulier**) : `est_avis` (est un avis d'imposition sur le revenu) · `pages_completes` [PRÉSENCE] (avis complet, non tronqué) · `nom_coherent` [CMP-FORM] (au moins un nom du foyer fiscal = un abonné) · `solvabilite` [RÈGLE] · `lisible` = tous bloquants. **Solvabilité** : `ratio = (mensualité TTC × 12) / revenus déclarés avant abattement 10%` (l'IA extrait `revenusAvantAbattement`, `Normaliser Avis` calcule le ratio **en dur**), bloquant si `ratio ≥ seuil` avec **seuil 4 %** (ou **7 %** si `form.batteriePhysique`). Revenus non lus ou mensualité absente → `a_verifier` (non bloquant). *(Avis au niveau **dossier**, 1 seul même en foyer multiple — cf. note ³.)*
