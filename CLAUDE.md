...
- **Types couverts & contrôles** (tous bloquants sauf mention ; `a_verifier` / IA indisponible → non bloquant) :
  ...
  - `KBIS` (clés `kbis` exploitant / `kbis-prop` propriétaire des murs, **pro**) : `est_kbis` (est un extrait Kbis / RCS — bloquant) · `raison_sociale` [CMP-FORM] (raison sociale lue = attendue selon la clé — **avertissement** non bloquant, tolère les formes juridiques SARL/SAS/SCI… via `norm()` dans `Normaliser Kbis`) · `lisible` (bloquant). `kbis-prop` n'est requis/affiché que si l'entreprise est **locataire** (`propT==='non'`). *(La CNI recto/verso du dirigeant, pro, reste gérée par le nœud **CNI** — pas par KBIS — via `cniBlockReason`/`renderDirigeantDocs`.)*
