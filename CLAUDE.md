...
- **Front** : registre `DOC_VERIFY` (clé document → `docType`) — actuellement `{ prop:'TITRE_PROPRIETE', maps/'map-pro':'GOOGLE_MAPS', calep/'cal-pro':'ETUDE_INSTALLATEUR', devis:'DEVIS', impots:'AVIS_IMPOT', bilans:'BILANS' }`. À l'ajout/retrait → `scheduleDocCheck(key)` → `doDocCheck(key)` POST `/api/drafts {action:'verifyid', docType, ids, refIds, form}` où `form` = `docFormContext()` (`{adresse, abonnes, duree, mensualite, batteriePhysique}` — `batteriePhysique` = `iType ∈ {pvb,bat}`, pour le seuil de solvabilité). Verdict dans `docVerdicts[key]` ; `renderDoc` **colore chaque vignette de fichier + affiche le motif dessus** (`_docChip(key,item,idx,vinfo)` + `docFileVinfo(key)` → tone `ok`/`warn`/`bad`), comme la CNI (`cniFileVinfo`). Coloration par fichier valable pour **tous** les documents. Blocage : `docsBlockReason()` (dans `slCreate`) si `statut==='ko'`.
