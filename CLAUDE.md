...
- **Documents — deux portées (scope)** : *(1)* **pièces du projet** (communes au dossier) = cartes, `docs[key]` = tableau `[{filename,size,id}]`. Deux modes : **multi-fichiers** (append via `onF(inp,key)` → `uploadFileFor(key,f)` ; bouton « Ajouter un document ») ou **mono-fichier** (`DOC_SINGLE`/`isSingleDoc(key)` → **un seul fichier**, dépôt = remplacement `uploadFileFor(key,f,{single:true})` avec `prevId`, `input` sans `multiple`, pas de bouton « Ajouter », « Remplacer » sur la vignette relance l'input). **Mono-fichier** = Google Maps, étude+calepinage, devis, titre de propriété, avis d'imposition, Kbis exploitant + propriétaire, fiche technique batterie (part **et** pro) ; **multi-fichiers** = facture élec (part+pro) et 3 bilans (plusieurs documents distincts légitimes, contrôle d'agrégat). Un « i » (`groupPagesTipHTML`, injecté par `upgradeDocZones`) rappelle sur les pièces mono-fichier de **regrouper toutes les pages dans un seul PDF** (Google Maps garde son `mapsTipHTML` d'exemple). **Retrait d'un fichier** (`removeFileFor`) : mono-fichier/CNI → la pièce se vide (`doDocCheck`/`doCniFaceCheck` sortent sans appel IA, verdict effacé) ; multi-fichiers → **re-vérification de l'ensemble restant** (nombre de pages/factures/exercices). *(2)* **pièces d'abonné** = **CNI par personne**, deux slots mono-fichier `docs['cni-recto-<uid>']` / `docs['cni-verso-<uid>']` (`uid` = identité **stable** de l'abonné, dans `p.abonnes[].uid`). Section « Documents des abonnés » reconstruite par `renderAbonneDocs()` (une carte par abonné : nom + email + « X/2 »), slots bâtis par `_buildCniSlotCard`. Upload en **remplacement** : `uploadFileFor(key,f,{single:true})` (envoie `prevId`). Rendu : `renderDocZone(key)` dispatche `isCniKey(k)`→`renderCniSlot` sinon `renderDoc` ; `upgradeDocZones()` transforme les `.uz` statiques (projet) en cartes ; builders partagés `_docDropzone`/`_docChip`. Progression via classe `.ok` sur les `.uz` (total dynamique = pièces projet + 2 × abonnés). Snapshot : chaque fichier mémorise `scope` (+ `face`/`abo` pour les CNI) ; à la reprise, `renderAbonneDocs()` rebranche `docs['cni-…-<uid>']` sur l'abonné via son `uid`.
...
- **Types couverts & contrôles** (tous bloquants sauf mention ; `a_verifier` / IA indisponible → non bloquant) :
  - `TITRE_PROPRIETE` (clé `prop` **particulier** / `prop-pro` **pro** ; branche via `form.profil`) : `type_document` (libellé « Documents acceptés (acte notarié ou taxe foncière) » ; acte notarié **OU** taxe foncière) · `adresse_form` [CMP-FORM] (libellé « Adresse en page 2 égale à celle de l'étude et du CRM ») · `nom_form` [CMP-FORM] (**particulier ET pro**, bloquant — libellé « Nom égal au Nom saisi dans l'étude et dans le CRM » ; en pro l'extraction compare le nom du propriétaire du titre à `form.raisonSociale`, + `form.proprietaireNom` si locataire `estProprietaire=false`) · `adresse_etude` [CMP-ÉTUDE] (**pro seulement**, **non bloquant** `a_verifier` — l'étude `cal-pro` est jointe en `refFiles` via `DOC_REFS['prop-pro']=['cal-pro']`, comparaison visuelle floue ; `etudeFournie=false` → `a_verifier`) · `pages_completes` (si taxe foncière ; libellé « Toutes les pages de la taxe foncière ») · `lisible`.
  ...
  - `ETUDE_INSTALLATEUR` (clé `calep`/`cal-pro`) : `puissance_kwc` (« Puissance de l'installation en kWc », 1ᵉʳ contrôle, bloquant présence — l'IA extrait `puissanceKwc`) · `calepinage` (présent + lisible) · `production_kwh` · `consommation_kwh` · `taux_autoconso` (« Taux d'autoconsommation direct ») · `taux_autoconso_batterie` (« Taux d'autoconsommation avec batterie virtuelle/physique, si présente », **non bloquant** `a_verifier` si absent — l'IA extrait `tauxAutoconsoBatterie`) · `courbe_charge` · `cash_flow` (« Cash flow (flux financier) ») · `lisible` (présence + extraction).
  - `DEVIS` (clé `devis` **particulier** / `dev-pro` **pro** — identiques, nœud agnostique du profil) : `signature` (« Signature du client ») · `total_ht_zero` **[BLOQUANT-DUR]** (libellé « Hormis le montant d'abonnement, les montants HT/TTC doivent être à 0€ (confidentialité client) » — **HT ET TTC** doivent valoir 0, **hors la ligne d'abonnement** ; l'IA extrait `totalHT` **et** `totalTTC` calculés hors abonnement ; `a_verifier` si l'un est absent) · `duree_abonnement` [CMP-FORM] · `materiel` (panneaux + onduleurs + quantités) · `lisible` = bloquants ; `montant_abonnement` [CMP-FORM] = **avertissement** (mensualité du formulaire = estimation ; le montant du devis est comparé à **HT ou TTC** — pro paie en HT, particulier en TTC). `docFormContext` fournit `duree` + `mensualite` {ht,ttc}.
