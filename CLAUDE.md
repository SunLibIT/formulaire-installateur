...
- **Verrou de séquence** : `DOC_GATE = { maps:'calep', 'map-pro':'cal-pro' }` — la copie **Google Maps se déduit de l'étude**, donc l'étude est placée **avant** dans le markup et la pièce Google Maps est **verrouillée** (`renderDoc` état `locked` : pastille « Verrouillé » + encadré ambre, dépôt désactivé, `uploadFileFor` refuse) tant que l'étude n'est pas déposée **et** analysée sans rejet. `docGateState(key)` : verrouillé si source absente / en cours d'analyse / en attente / `ko` ; débloqué dès `ok`/`a_verifier`, **ou** si l'IA est indisponible après tentative (`_docTried` → dégradé, ne bloque jamais). `refreshGatedBy(gateKey)` (appelé dans `doDocCheck`) re-rend la pièce verrouillée quand la source change.
